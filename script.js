...
function getGMTShort(timezone) {
    ...
    if (!offset.includes('+') && !offset.includes('-')) {
        offset = offset.replace('GMT', 'GMT+00:00');
    } else {
        offset = offset.replace('GMT', 'GMT ');
    }
    return offset;
}
...
function populateTimeZoneList() {
    ...
    let localOffset = localParts.find(p => p.type === 'timeZoneName').value;
    if (!localOffset.includes('+') && !localOffset.includes('-')) {
        localOffset = localOffset.replace('GMT', 'GMT+00:00');
    } else {
        localOffset = localOffset.replace('GMT', 'GMT ');
    }
